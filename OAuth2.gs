...
Service_.prototype.setPropertyStore = function(propertyStore) {
  if (propertyStore && propertyStore != null && typeof propertyStore != 'undefined') {
    this.propertyStore_ = propertyStore;
  } else {
    this.propertyStore_ = PropertiesService.getScriptProperties();
  }
  return this;
};
...
Service_.prototype.reset = function() {
  ...
  //});
  PropertiesService.getScriptProperties().deleteProperty(this.getPropertyKey_(this.serviceName_));
};
...
Service_.prototype.getToken_ = function() {
  ...
  if (!token) {
    token = PropertiesService.getScriptProperties().getProperty(key);
  }
  if (token) {
    if (this.cache_) {
...
function isEmpty_(value) {
  return value === null || value === undefined || ((_.isObject(value) || _.isString(value)) && _.isEmpty(value));
}
